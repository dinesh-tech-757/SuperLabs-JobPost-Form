    ...
    return (
        <main className="px-16 py-3 grid grid-cols-[1fr_4.5fr] gap-4">
            <section className="py-2">
                <h3 className="font-medium text-gray-500">{filteredJobs.length} jobs</h3>
                ...
                <div className="py-2">
                    ...
                    <div>
                        <h3 className="font-medium flex gap-2 text-blue-500 text-lg py-3 items-center">Locations</h3>
                        {location.map((loc, idx) => (
                            <label key={idx} className="flex gap-3 py-1 items-center text-sm text-gray-600 mb-1">
                                <input
                                    type="checkbox"
                    ...
                    <div>
                        <h3 className="font-medium flex gap-2 text-blue-500 text-lg py-3 items-center">Job Category</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
                        {filteredJobs.length > 0 ? (
                            filteredJobs.map((item, index) => (
                                <div key={index} className="border relative border-gray-300 hover:shadow-lg h-max shadow-blue-100 rounded-lg py-6 px-6 pb-10">
                                    <Link to={`/job/${item.job_id}`}><h1 className="text-blue-500 text-xl inline font-semibold py-2 hover:cursor-pointer hover:underline">{item.job_title}</h1></Link>
                                    <h3 className="text-xl text-gray-500">{item.job_experience_level}+ years</h3>
                                    <div className="grid grid-cols-[1fr_1fr_1fr] gap-2 py-4">
                                        {item.job_technical_skills.map((skill, index) => (
                                            <p key={index} className="text-blue-500 bg-blue-100 font-semibold rounded-full px-2 text-nowrap overflow-hidden">{skill}</p>
                                        ))}
                                    </div>
                                    <p className="bg-gray-200 h-4 rounded-full"></p>
                                    <div className="flex justify-between pt-6 font-medium text-xl">
                                        <p className="flex items-center gap-1"><CiLocationOn className="text-blue-500 font-bold text-xl" />{item.job_location}</p>
                                        <p className="flex items-center gap-1"><MdWorkOutline className="text-blue-500 font-bold text-xl" />{item.job_type}</p>
                                    </div>
                                    <div className="text-red-500 absolute bottom-1 right-4">
                                        <Link to={`/job/${item.job_title}`}><p className="flex items-center">View full details <FaCaretRight /></p></Link>
                                    </div>
                                </div>
                            ))
                        ) : (
